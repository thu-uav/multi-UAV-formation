...
def main(cfg):
    ...
    def _solve(weight, cnt):

        run = init_wandb(cfg)
        setproctitle(run.name)
        run.config.update({
            "morl_formation_coeff": weight[0],
            "morl_obstacle_coeff": weight[1],
            "morl_forward_coeff": weight[2],
            "morl_smooth_coeff": weight[3],
        })

        base_env.morl_formation_coeff = weight[0]
        base_env.morl_obstacle_coeff = weight[1]
        base_env.morl_forward_coeff = weight[2]
        base_env.morl_smooth_coeff = weight[3]
        ...
        value = np.array([
            last_info["eval/stats.morl_formation"], 
            last_info["eval/stats.morl_obstacle"],
            last_info["eval/stats.morl_forward"],
            last_info["eval/stats.morl_smooth"],
            ], dtype=np.float32)
    ...
    for w in weight:
        value, name = _solve(w, 0)
        weights.append(w)
        values.append(value)
        names.append(name)

    np.save('weight', weights)
